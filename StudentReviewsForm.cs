        ...
        private void LoadReviews()
        {
            try
            {
                ...
                if (reviews.Count > 0)
                {
                    foreach (var review in reviews)
                    {
                        ...
                        if (!string.IsNullOrEmpty(review.Rating))
                        {
                            if (review.Rating.Contains("מצוין"))
                            {
                                ...
                                row.DefaultCellStyle.ForeColor = Color.FromArgb(184, 134, 11);
                    ...
                    // שימוש בלוגיקת חישוב ממוצע
                    double? average = StudentReviewsLogic.CalculateAverageScore(reviews);
                    labelAverageRating.Text = StudentReviewsLogic.GetAverageText(average);
                    avgRatingIcon.Text = StudentReviewsLogic.GetAverageIcon(average);

                    if (average.HasValue)
                    {
                        if (average >= 2.5)
                            averageRatingPanel.BackColor = Color.FromArgb(245, 158, 11);
                        else if (average >= 1.5)
                            averageRatingPanel.BackColor = Color.FromArgb(16, 185, 129);
                        else
                            averageRatingPanel.BackColor = Color.FromArgb(239, 68, 68);
                    }
                    else
                    {
                        averageRatingPanel.BackColor = Color.FromArgb(16, 185, 129);
                    ...
                    // שימוש באייקון לפי דירוג אחרון
                    var latestReview = reviews.FirstOrDefault();
                    summaryIcon.Text = StudentReviewsLogic.GetLatestRatingIcon(latestReview?.Rating);
                }
                else
                {
