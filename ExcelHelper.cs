...
namespace Exam_Questioner
{
    public static class ExcelHelper
    {
        ...
        {
            // ✅ הוספת בדיקות כפילות לפני כל פעולה
            if (UsernameExists(username) || IDExists(id))
                return false;
        ...
        public static string GetUserEmail(string username, string role)
        {
            if (!File.Exists(filePath)) return "";
            ...
            using (var workbook = new XLWorkbook(filePath))
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    string u = row.Cell(1).GetString(); // Username
                    string r = row.Cell(6).GetString(); // Role

                    if (u == username && r == role)
                        return row.Cell(5).GetString(); // Email
                }
            }
            return "";
        }
